...
function CLASS:SetOffset(offset)
	if (self.IsEnabled == false) then
...
function CLASS:Update()
	...
	--// Address mouse input //--
	local mouseDelta = USER_INPUT_SERVICE:GetMouseDelta() * activeCameraSettings.Sensitivity * 2.25
	...
	if (humanoidRootPart ~= nil) then
		...
		if (raycastResult ~= nil and raycastResult.Instance.CanCollide == true) then
	...
	if (gameProcessedEvent == false) and (singleton.IsEnabled == true) then
		if (inputObject.KeyCode == Enum.KeyCode.LeftControl) then
			if (singleton.IsEnabled == true) then
				USER_INPUT_SERVICE.MouseIconEnabled = singleton.IsMouseSteppedIn
